...
`translate-shield` is what this study produced: dependency-free, 13.3 kB packed, entry points `translate-shield` and `translate-shield/react`. It mirrors the new value into the translator's wrapper rather than restoring the original, which is where the 0ms comes from. On Edge and Firefox it has nothing to do.
