...
`translate-shield` is what this study produced: dependency-free, about 15 kB packed, entry points `translate-shield` and `translate-shield/react`. It is on npm as [`translate-shield`](https://www.npmjs.com/package/translate-shield), and the source, every raw recording and the specs that produced them are at [github.com/alievdavlat/translate-shield](https://github.com/alievdavlat/translate-shield). A [live demo](https://google-translate-simulation.netlify.app/) runs the unprotected and protected versions side by side in the reader's own browser, in two documents rather than one, because the patch covers a whole document and a single page could not host an honest control. It mirrors the new value into the translator's wrapper rather than restoring the original, which is where the 0ms comes from. On Edge and Firefox it has nothing to do.
